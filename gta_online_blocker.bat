...
powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "$env:BATCH_DIR='%~dp0'; iex ((Get-Content -Path '%~f0' -Raw -Encoding UTF8) -replace '(?s)^.*?:ps_start\r?\n','')"
exit /b
...
$btnLaunchVpn.Add_Click({
    ...
    if ($selected -match "\(([\d\.]+)\)") {
        ...
        $forceBindPath = Join-Path $env:BATCH_DIR "ForceBindIP64.exe"
        if (-not (Test-Path $forceBindPath)) {
            $msg = "Утилита ForceBindIP64.exe не найдена в папке со скриптом.`n`nВам необходимо скачать её с официального сайта (r1ch.net/projects/forcebindip) и положить ForceBindIP64.exe и BindIP64.dll в папку: `n$env:BATCH_DIR"
